...
if [ "$MENU_PID" -eq 0 ]; then
    # 顶级菜单
    JSON="{\"menu_pid\":0,\"menu_name\":\"$MENU_NAME\",\"icon\":\"\",\"path\":\"$PATH\",\"sort\":50,\"status\":1,\"is_show\":1}"
else
    # 子菜单
    JSON="{\"menu_pid\":$MENU_PID,\"menu_name\":\"$MENU_NAME\",\"icon\":\"\",\"path\":\"$PATH\",\"sort\":50,\"status\":1,\"is_show\":1}"
fi
